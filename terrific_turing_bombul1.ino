...
void loop() {
  ...
  if (digitalRead(button) == LOW) { //if Value read of the button ==LOW:
    pos=180;  //increases the value of the "pos" variable each time the push button of the left is pressed
    servo.write(pos); //servo goes to variable pos
  }
  if (digitalRead(button1) == LOW) { //if Value read of the button ==LOW:
    pos=0;  //decreases the value of the "pos" variable each time the push button of the right is pressed
    servo.write(pos); //servo goes to variable pos
  }
}